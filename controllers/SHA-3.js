 ...
 * While it is fully tested, it is not at all optimized and is not recommended for production use.
 *
...
class Sha3 {
    ...
    static keccak1600(r, c, M, options) {
        ...
        for (let i = 0; i < msg.length; i += blocksize) {
            for (let j = 0; j < r / w; j++) {
                const i64 = (BigInt(msg.charCodeAt(i + j * 8 + 0)) << 0n)
...
// Your hashing function using ES module syntax
export const sha3Hash = async (request, response) => {
    ...
    try {
        const hash256 = Sha3.hash256(text);
        const hash512 = Sha3.hash512(text);
        response.json({
            hash256,
            hash512,
